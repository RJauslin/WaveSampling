...
We use the data set `meuse` from the package `sp` describe by the following statement. "This data set gives locations and topsoil heavy metal concentrations, along with a number of soil and landscape variables at the observation locations, collected in a flood plain of the river Meuse, near the village of Stein (NL)". We proposed to generate the inclusion probabilities proportional to the copper concentration. As it is explained by @grafstrom:tille:2013, this data set has a strong spatial correlation.
...
pik <- inclusionprobabilities(meuse$copper,50)
```
...
ggplot() +
  geom_polygon(data = data.frame(x = meuse.riv[,1],y = meuse.riv[,2]),aes(x = x,y = y),fill = "lightskyblue2")+
  geom_voronoi(data = meuse[which(s == 1),],
               aes(x = x,y = y,fill = v),
               outline =as.data.frame(meuse.area),
               size = 0.1,alpha = 0.8,colour = "black")+
  geom_point(data = meuse,aes(x = x,y = y,size = copper),shape = 1,stroke = 0.3)+
  geom_point(data = meuse[which(s == 1),] ,aes(x = x,y = y,size = copper),shape = 16,alpha = 1)+
  labs(x = NULL,
       y = NULL,
       title = "Meuse river with Voronoï polygon",
